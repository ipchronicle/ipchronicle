...
func TestLoadRuntimeConfigDefaults(t *testing.T) {
	...
	if configuration.ExternalOrigin != nil || len(configuration.TrustedProxies) != 0 {
...
func TestLoadRuntimeConfigOverrides(t *testing.T) {
	...
	t.Setenv("IPCHRONICLE_EXTERNAL_URL", "HTTPS://EXAMPLE.COM:8443/")
	...
	if configuration.ExternalOrigin == nil || configuration.ExternalOrigin.String() != "https://example.com:8443" {
		t.Fatalf("external origin = %v", configuration.ExternalOrigin)
	}
...
func TestLoadRuntimeConfigRejectsInvalidValues(t *testing.T) {
	tests := []struct {
		...
		{name: "external URL path", key: "IPCHRONICLE_EXTERNAL_URL", value: "https://example.com/ipchronicle"},
		{name: "external URL credentials", key: "IPCHRONICLE_EXTERNAL_URL", value: "https://owner@example.com"},
