...
func TestNetworkProxyCredentialsAndReferencedConfiguration(t *testing.T) {
	...
	if configuration.SchemaVersion != 7 || configuration.Revision != 2 || len(configuration.DiscoveryPaths) != 2 || len(configuration.ProbeTargets) != 0 || len(configuration.Proxies) != 1 || len(configuration.DiscoveryServices.IPv4) < 2 {
