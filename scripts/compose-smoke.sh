...
jq -n --arg key "$registration_key" '{registrationKey:$key,metadata:{hostname:"smoke-node",agentVersion:"dev",operatingSystem:"linux",architecture:"amd64",physicalMemoryBytes:536870912,capabilities:["control-v1","configuration-v9","complete-probe-v1"]}}' | \
  curl --fail --silent --show-error \
    --header 'Content-Type: application/json' \
    --data-binary @- \
    "$base_url/api/v1/agent/enroll" >"$registration_file"
agent_credential="$(jq -er '.credential' "$registration_file")"
jq -n '{appliedConfigurationRevision:0,metadata:{hostname:"smoke-node",agentVersion:"dev",operatingSystem:"linux",architecture:"amd64",physicalMemoryBytes:536870912,capabilities:["control-v1","configuration-v9","complete-probe-v1"]}}' | \
  curl --fail --silent --show-error \
    --header 'Content-Type: application/json' \
...
if ! jq -e '.schemaVersion == 9 and .revision == 1 and .enabled == true and (.historyGeneration | length == 64) and .discoveryPaths == [] and .probeTargets == [] and .proxies == [] and .probeSchedule.enabled == true and .probeSchedule.cron == "0 0 0 * * *" and .probeSchedule.timezone == "UTC" and .probeLowMemoryOverride == false' "$configuration_file" >/dev/null; then
  echo "Agent configuration did not match the expected initial configuration-v9 contract" >&2
  jq . "$configuration_file" >&2
  rm -f "$configuration_file"
  exit 1
fi
rm -f "$configuration_file"
jq -n '{appliedConfigurationRevision:1,metadata:{hostname:"smoke-node",agentVersion:"dev",operatingSystem:"linux",architecture:"amd64",physicalMemoryBytes:536870912,capabilities:["control-v1","configuration-v9","complete-probe-v1"]}}' | \
  curl --fail --silent --show-error \
    --header 'Content-Type: application/json' \
    --header "Authorization: Bearer $agent_credential" \
    --data-binary @- \
