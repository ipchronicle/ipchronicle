...
- Preserve upgrades from `v0.1.1` for `config.db`, `master.key`, `history.db`,
  the `./data/config` and `./data/history` Compose layout, and Agent state under
  `/var/lib/ipchronicle-agent`.
- Center schema changes use new ordered migrations. Do not rewrite a released
  migration. Agent state changes must preserve the update and rollback boundary.
- Persisted-format changes require fixtures generated by the applicable old
  release or fixed old-version fixtures. Verify configuration, history,
  identities, credential references, and offline queues rather than recreating
  an alleged old state with current serialization code.
- Development builds, release candidates, and `v0.1.0` are not supported
  upgrade sources. Do not add migration or fallback branches for them.
- Corrupt or unsupported stable data must fail explicitly. Never silently
  delete, rebuild, replace, downgrade, or reinterpret it.
