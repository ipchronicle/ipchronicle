...
打开节点的**公网 IP** 页面，管理该节点可到达的规范公网地址。点击右上角的
**管理代理**，可以在弹窗中添加、编辑、启用、禁用或删除只属于当前节点的 HTTP、
HTTPS 和 SOCKS5 代理。保存新代理或修改代理后，Agent 会自动检查它的 IPv4 和
IPv6 公网出口。

代理密码加密保存在 `config.db`，只会发送给所属节点的 Agent，且不会再次显示。
编辑时密码留空会保留原值，除非明确选择**清除密码**。禁用代理会停止通过它发现
和探测，但保留配置。
...
- 节点代理自动检查单栈或双栈公网出口，因为这些出口无法从直连网络清单推断。
...
`v0.1.1` 已于 2026-09-04 投入正式使用，是首个持久数据兼容基线。后续同一主
版本的受支持升级必须保留它的配置库、主密钥、历史库、Compose 数据目录和 Agent
状态。`v0.1.1` 不迁移开发版、候选版或 `v0.1.0` 数据；无法读取的数据会明确
报错。
...
Center 开始服务前会运行数据库迁移。涉及持久格式的版本会在对应版本说明中列出
升级和回滚要求，并验证从 `v0.1.1` 及其他仍受支持稳定版本升级。
